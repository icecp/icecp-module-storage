...
class TagMessage extends BaseMessage {
     ...
     */
    @JsonCreator
    TagMessage(String[] tags, long[] ids) {
        this.tags = tags;
        this.ids = ids;
    }
    ...
    public Object onCommandMessage(StorageModule context) throws StorageModuleException {
        ...
        for (long id : ids) {
            for (String tagName : tags) {
                try {
                    Set<Id> taggedIds = context.getStorageProvider().tag(Queries.fromId(id), new Tag(tagName));
                    vertexIdentifiers.addAll(taggedIds.stream().map(Id::value).collect(Collectors.toList()));
                } catch (TaggingOperationException e) {
                    throw new StorageModuleException(String.format("Unable to set tag '%s' on id '%d'.", tagName, id), e);
                }
            }
        }
    ...
    public String toString() {
        return "TagMessage{TagName=" + Arrays.toString(tags) + "Id=" + Arrays.toString(ids) + "} " + super.toString();
    }
}
